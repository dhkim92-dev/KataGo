...
* @brief Add Channel Bias NC FP32 compute shader (ReLU activation)
* Adds bias to each channel with ReLU: accum[n,c] = max(0, accum[n,c] + bias[c])
...
void main(uint3 DTid : SV_DispatchThreadID)
{
    ...
    if (idx < totalSize) {
        ...
        g_accum[idx] = RELU(g_accum[idx] + g_bias[c]);
    }
}
