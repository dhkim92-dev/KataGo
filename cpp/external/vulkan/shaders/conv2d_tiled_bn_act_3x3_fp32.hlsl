/**
 * @file conv2d_tiled_bn_act_3x3_fp32.hlsl
 * @author dhkim92.dev@gmail.com
 * @brief Optimized 3x3 Conv2D with fused BN->Act->Conv for KataGo.
 *        Fixed 3x3 kernel enables aggressive loop unrolling and constant folding.
 *
 * Push constants: Conv2dTiledBnActParams
 * Buffers:
 *  - inputBuf : StructuredBuffer<float> (NCHW)
 *  - filters  : StructuredBuffer<float> (oc, ic, 3, 3)
 ...
 *
 * Optimizations:
 *  - 3x3 kernel constants enable compile-time optimizations
 *  - Full loop unrolling for 9 filter positions
...
// 3x3 kernel constants - enables compiler optimizations
#define FILTER_SIZE 3
#define FILTER_HALF 1
#define FHFW 9
...
struct Conv2dTiledBnActParams
{
    ...
    uint nnYLen;
    uint nnXLen;
    uint filterH;    // Always 3
    uint filterW;    // Always 3
    uint activation;
};
    ...
    for (uint kb = 0u; kb < numKBlocks; ++kb) {
        ...
        for (uint i = tid; i < inputEltsPerTile; i += numThreads) {
            ...
            if (k < K && x < params.nnXLen && outY < params.nnYLen && batch < params.batchSize) {
                // Decode k -> (ic, fh, fw) for 3x3: k = ic * 9 + fh * 3 + fw
                uint ic = k / FHFW;
                uint rem = k - ic * FHFW;  // Faster than k % 9
                uint fh = rem / FILTER_SIZE;
                uint fw = rem - fh * FILTER_SIZE;  // Faster than rem % 3
                ...
                if (inX >= 0 && inX < int(params.nnXLen) && inY >= 0 && inY < int(params.nnYLen)) {
                    ...
                    // Use cached mask (rem = fh * 3 + fw = filter position)
