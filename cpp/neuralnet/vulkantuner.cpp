...
namespace {
  ...
  CandidateResult evaluateCandidate(
    ...
    try {
      // TODO: pass real parameters Transformer, useFP16, useTensorcore
      pipelines = new vk_shader::ComputePipelines(device->device, params, false ,false, -1, -1);
...
VulkanTuneParams VulkanTuner::loadOrAutoTune(
  ...
  if(loadedModel == nullptr)
    throw StringError("Vulkan tuner received a null loaded model");
  const ModelInfoForTuning modelInfo = ModelInfoForTuning::ofDesc(loadedModel->modelDesc);
